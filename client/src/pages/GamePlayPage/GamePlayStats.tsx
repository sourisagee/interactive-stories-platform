...
export default function GamePlayStats({ currentNode, restartKey }: GamePlayStatsProps) {
  ...
  useEffect(() => {
    ...
    const braveKeywords =
      /сразить|сразиться|атак|напасть|остаться|жертв|пожертв|защитить|щит|страж|подкрепление/i;
    ...
    const respectPlus =
      /спасти|защитить|выбрать других|пожертвовать собой|пожертвовать силой|спасён город|ордена|герой|любовь/i;
    const respectMinus =
