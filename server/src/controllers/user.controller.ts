...
export class UserController {
  ...
  static async signUp(req: Request, res: TypedResponse): Promise<void> {
    ...
    try {
      ...
      if (found) {
        res.status(400).json(formatResponse(400, 'User with this email already exists', null, 'User with this email already exists'));
        return;
      }
      ...
      res
        ...
        .json(formatResponse(201, 'Registration successful', { user, accessToken }, null));
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Internal server error';
      res.status(500).json(formatResponse(500, 'Internal server error', null, msg));
    }
  }
  ...
  static async signIn(req: Request, res: TypedResponse): Promise<void> {
    ...
    try {
      const userFound = await UserService.getUserByEmail((email as string)?.trim().toLowerCase());
      if (!userFound) {
        res.status(400).json(formatResponse(400, 'User with this email not found', null, 'User with this email not found'));
        return;
      }
      const valid = await bcrypt.compare(password, userFound.password);
      if (!valid) {
        res.status(400).json(formatResponse(400, 'Invalid password', null, 'Invalid password'));
        return;
      }
      ...
      res
        .status(200)
        .cookie('refreshToken', refreshToken, cookieConfig)
        .json(formatResponse(200, 'Sign in successful', { user, accessToken }, null));
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Internal server error';
      res.status(500).json(formatResponse(500, 'Internal server error', null, msg));
    }
  ...
  static async getById(req: Request, res: TypedResponse): Promise<void> {
    ...
    if (Number.isNaN(id)) {
      res.status(400).json(formatResponse(400, 'Invalid user id', null, 'Invalid user id'));
      return;
    }
    try {
      const user = await UserService.getById(id);
      if (!user) {
        res.status(404).json(formatResponse(404, 'User not found', null, 'User not found'));
        return;
      }
  ...
  static async updateUserById(req: Request, res: TypedResponse): Promise<void> {
    ...
    if (currentId !== id) {
      res.status(403).json(formatResponse(403, 'You can only update your own profile', null, 'Forbidden'));
      return;
    }
    try {
      const updated = await UserService.update(id, req.body as UpdateUserData);
      if (updated === null) {
        res.status(400).json(formatResponse(400, 'Email already in use', null, 'Email already in use'));
        return;
      }
  ...
  static async deleteUserById(req: Request, res: TypedResponse): Promise<void> {
    ...
    if (currentId !== id) {
      res.status(403).json(formatResponse(403, 'You can only delete your own account', null, 'Forbidden'));
      return;
    }
    try {
